...
//	}
	...
	public void createVoucherTypeA() {
		...
		for (int i = 0; i < VoucherAmount; i++) {
			...
			// 엔터티를 먼저 저장한 후에 voucherSeq를 얻을 수 있음
		    Voucher savedVoucher = vRepo.save(voucher);
		    savedVoucher.setVoucherCode(VoucherType+generateVoucherCode(savedVoucher.getVoucherSeq(), "dining10"));		    
		    vRepo.save(savedVoucher);
		} 
		...
		for (int i = 0; i < VoucherAmount; i++) {
			...
			// 엔터티를 먼저 저장한 후에 voucherSeq를 얻을 수 있음
		    Voucher savedVoucher = vRepo.save(voucher);
		    savedVoucher.setVoucherCode(VoucherType+generateVoucherCode(savedVoucher.getVoucherSeq(), "cultureBL"));		    
		    vRepo.save(savedVoucher);
		} 
	...
	public void createVoucherTypeB() {
		...
		for (int i = 0; i < VoucherAmount; i++) {
			...
			// 엔터티를 먼저 저장한 후에 voucherSeq를 얻을 수 있음
		    Voucher savedVoucher = vRepo.save(voucher);
		    savedVoucher.setVoucherCode(VoucherType+generateVoucherCode(savedVoucher.getVoucherSeq(), "dining7"));		    
		    vRepo.save(savedVoucher);
		} 
		...
		for (int i = 0; i < VoucherAmount; i++) {
			...
			// 엔터티를 먼저 저장한 후에 voucherSeq를 얻을 수 있음
		    Voucher savedVoucher = vRepo.save(voucher);
		    savedVoucher.setVoucherCode(VoucherType+generateVoucherCode(savedVoucher.getVoucherSeq(), "cultureRED"));		    
		    vRepo.save(savedVoucher);
		} 
